// Unitests for posterior sampling when the field is not constrained to
...
using L = pift::GaussianLikelihood<F, FField>;
using UEGradWH = pift::UEIntegralGradWH<F, H, FField, Domain>;
...
int main(int argc, char* argv[]) {
  ...
  // Add some random noise on the boundary values
  const int num_obs = 2;
  F x_obs[num_obs];
  F y_obs[num_obs];
  ...
  // We only sample at the boundary
  std::copy(bounds, bounds + 2, x_obs);
  ...
  const int num_warmup = 40'000'000;
  const int num_samples = 40'000'000;
