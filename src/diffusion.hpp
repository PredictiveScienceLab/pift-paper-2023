...
template<typename T>
class Example1Hamiltonian : Hamiltonian {
  private:
  ...
  public:
    Example1Hamiltonian(const T& beta, const T& kappa=0.25) : 
      Hamiltonian(0), beta(beta), kappa(kappa)
  {}
    ...
    inline T density(const T* x, const T* prolong_phi, const T* theta) const 
    {
    ...
    inline T grad_phi_prolong(
      const T* x, const T* prolong_phi, const T* theta,
      T* out
    ) const
    {
      const T phi = prolong_phi[0];
      const T phi_primve = prolong_phi[1];
      const T q_x = q(x);
      out[0] = -beta * q_x;
