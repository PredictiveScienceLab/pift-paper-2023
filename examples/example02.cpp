...
int main(int argc, char* argv[]) {
  if(argc != 6) {
    std::cerr << "Usage:\n\t" << argv[0] 
        << " <GAMMA> <CONFIG_FILE> <N> <SIGMA> <ID>" << std::endl;
    exit(1);
  }

  // Read gamma from the command line
  const F gamma = static_cast<F>(std::stod(argv[1]));
  ...
  // The output prefix
  char buffer2[256];
  snprintf(buffer2, 256, "gamma=%1.2e_%s", gamma, buffer);
